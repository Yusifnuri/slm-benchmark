...
# phi-4-mini-instruct — Code generation adapter

A LoRA adapter that specialises `microsoft/Phi-4-mini-instruct` (3.80 B parameters) for a single enterprise task: it completes a Python function so that it passes the reference unit tests.
...
|---|---|
| pass@1 | **0.4287** |
| Mean latency, batch 1 | 2200 ms |
| Cost per 1M generated tokens | USD 9.53 |
...
model = PeftModel.from_pretrained(base, "<your-hf-username>/phi-4-mini-instruct_code_generation")
