...
# Llama-3.2-3B-Instruct — Financial sentiment adapter

A LoRA adapter that specialises `meta-llama/Llama-3.2-3B-Instruct` (3.21 B parameters) for a single enterprise task: it classifies a financial sentence as negative, neutral or positive.
...
|---|---|
| Accuracy | **0.76** |
| Mean latency, batch 1 | 468 ms |
| Cost per 1M generated tokens | USD 16.22 |
...
model = PeftModel.from_pretrained(base, "<your-hf-username>/Llama-3.2-3B-Instruct_financial_sentiment")
